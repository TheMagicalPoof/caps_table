...
const bundledModelPath = `${base}/models/cap-detector.onnx`;
...
async function readBundledModel() {
  const response = await fetch(bundledModelPath);
  if (!response.ok) {
    throw new Error(`Bundled ONNX model is unavailable: ${response.status}`);
  }
...
async function getSession() {
  if (!cachedSessionPromise) {
    cachedSessionPromise = (async () => {
      ...
      const modelBuffer = storedModel ? new Uint8Array(storedModel.buffer) : await readBundledModel();
